...
def get_payment_url(user_id, amount, requests_count):
    ...
    return (f"https://pay.freekassa.ru/?m={config.MERCHANT_ID}&oa={amount}¤cy=RUB&o={order_id}&s={sign}"
            f"&us_user_id={user_id}&us_requests_count={requests_count}"), sign_data
...
async def payment_handler(message: types.Message):
    ...
    payment_url, sign_data = get_payment_url(user.id, amount, requests_count)
    keyboard_markup = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton("Оплатить", url=payment_url)]]
    )

    await message.reply(f"Для оплаты нажмите на кнопку ниже:\n\nSign Data: {sign_data}", reply_markup=keyboard_markup)
